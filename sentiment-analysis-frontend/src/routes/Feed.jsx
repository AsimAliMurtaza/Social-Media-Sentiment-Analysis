...
import { Paper, Grid, Typography, IconButton, Divider, Button, TextField } from '@mui/material';
import { ThumbUp, Favorite, MoodBad, SentimentVeryDissatisfied, Mood, EmojiEmotions, Comment } from '@mui/icons-material';
        ...
        {posts.map((post) => (
          <div key={post.id} style={{ marginBottom: '20px', display: 'flex', alignItems: 'center' }}>
            <div style={{ flex: 1 }}>
              ...
              <div>
                <IconButton disabled={reactedPosts.includes(post.id)} onClick={() => reactToPost(post.id, 'like')}>
                  <ThumbUp />
                </IconButton>
                <IconButton disabled={reactedPosts.includes(post.id)} onClick={() => reactToPost(post.id, 'love')}>
                  <Favorite />
                </IconButton>
                <IconButton disabled={reactedPosts.includes(post.id)} onClick={() => reactToPost(post.id, 'sad')}>
                  <MoodBad />
                </IconButton>
                <IconButton disabled={reactedPosts.includes(post.id)} onClick={() => reactToPost(post.id, 'angry')}>
                  <SentimentVeryDissatisfied />
                </IconButton>
                <IconButton disabled={reactedPosts.includes(post.id)} onClick={() => reactToPost(post.id, 'wow')}>
                  <Mood />
                </IconButton>
                <IconButton disabled={reactedPosts.includes(post.id)} onClick={() => reactToPost(post.id, 'haha')}>
                  <EmojiEmotions />
                </IconButton>
              </div>
              <Typography variant="body2" style={{ marginTop: '10px', color: '#888' }}>
                Reactions: 
            ...
            <div>
              ...
              {commentingOnPostId === post.id && (
                <div style={{ display: 'flex', marginTop: '10px' }}>
                  ...
                  />
                  <Button variant="contained" color="primary" onClick={() => handleSubmitComment(post.id)}>
                    Submit
                  </Button>
                </div>
              )}
